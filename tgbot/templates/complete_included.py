...
def complete_included_kb(page=0):
    ...
    for keyphrases in list(included_complete_deals)[start_offset:end_offset]:
        ...
        rows.append([
            ...
            InlineKeyboardButton(text="🗑️", callback_data=calls.DeleteIncludedCompleteDeal(index=included_complete_deals.index(keyphrases)).pack()),
        ])

    if total_pages > 1:
        buttons_row = []
        btn_back = InlineKeyboardButton(text="←", callback_data=calls.IncludedCompleteDealsPagination(page=page-1).pack()) if page > 0 else InlineKeyboardButton(text="🛑", callback_data="null_answer")
        buttons_row.append(btn_back)
        
        btn_pages = InlineKeyboardButton(text=f"{page+1}/{total_pages}", callback_data="null_answer")
        buttons_row.append(btn_pages)

        btn_next = InlineKeyboardButton(text="→", callback_data=calls.IncludedCompleteDealsPagination(page=page+1).pack()) if page < total_pages - 1 else InlineKeyboardButton(text="🛑", callback_data="null_answer")
