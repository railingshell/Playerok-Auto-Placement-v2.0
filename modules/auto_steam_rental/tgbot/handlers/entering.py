...
@router.message(states.STEAMRENT_AddAccountStates.entering_login, F.text)
async def handler_entering_login(message: types.Message, state: FSMContext):
    try:
        ...
        await state.set_state(states.STEAMRENT_AddAccountStates.entering_password)
...
@router.message(states.STEAMRENT_AddAccountStates.entering_password, F.text)
async def handler_entering_password(message: types.Message, state: FSMContext):
    try:
        ...
        await state.set_state(states.STEAMRENT_AddAccountStates.entering_mafile)
        await throw_float_message(
            state,
            message,
...
@router.message(states.STEAMRENT_AddAccountStates.entering_mafile)
...
@router.message(states.STEAMRENT_SettingsStates.entering_duration, F.text)
async def handler_entering_duration(message: types.Message, state: FSMContext):
    try:
        value = message.text.strip()
        if not value.isdigit() or int(value) < 1:
            raise Exception("❌ Введите целое число часов")
        config = sett.get("config")
        config["rental"]["duration_hours"] = int(value)
        sett.set("config", config)
        await state.set_state(None)
        await throw_float_message(
            state,
            message,
            templ.float_text("✅ Длительность аренды обновлена"),
...
@router.message(states.STEAMRENT_SettingsStates.entering_api_key, F.text)
async def handler_entering_api_key(message: types.Message, state: FSMContext):
    try:
        value = message.text.strip()
        config = sett.get("config")
        config["steam"]["api_key"] = value
        sett.set("config", config)
        await state.set_state(None)
        await throw_float_message(
            state,
            message,
            templ.float_text("✅ API ключ сохранён"),
            main_templ.back_kb(calls.STEAMRENT_MenuNavigation(to="settings").pack())
        )
    except Exception as e:
        if e is not TelegramAPIError:
