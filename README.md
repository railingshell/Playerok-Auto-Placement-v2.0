...
- 🧩 **Модули тоже под защитой:** секреты в конфигах модулей (LZT-токен, Steam
  api_key) шифруются пополево, а файлы данных модулей (`accounts.json`,
  `rentals.json`, `purchases.json` — с логинами, паролями и maFile) — целиком.
  Всё прозрачно и обратно совместимо со старыми данными.
...
**💾 Бэкап** (кнопки «Создать бэкап», «Отправить архив в чат» и «Список бэкапов»).

> ⚠️ Кнопка «Отправить архив в чат» загружает архив в Telegram — он содержит ключ
> шифрования и токены. Используйте её только в личном чате с ботом и не пересылайте
> файл посторонним.
